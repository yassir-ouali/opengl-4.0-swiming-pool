...
void camera::Control(pipeline& trf)
{
	if (mi)
	{
		int MidX = 320;
	...
	trf.matrixMode(VIEW_MATRIX);

	trf.rotateX(camPitch);
...
void camera::UpdateCamera(pipeline& trf)
{
	trf.matrixMode(MODEL_MATRIX);
	trf.translate(-loc.x, -loc.y, -loc.z);
